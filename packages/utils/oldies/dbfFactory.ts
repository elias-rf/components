// ts-nolint
import fsys from "fs";
import trimExString from "../string/trimExString";
 ...
 * @memberOf Dbf
 */
function recordOffset(recNum: number, header: any) {
 ...
 * @constructor
 */
function dbfFactory(dbfFile: string, fs = fsys) {
  /**
   ...
   * @memberOf Dbf
   */
  let fd: number; // descritor do arquivo dbf
  const headerDbf = {
    versao: 0,
    update: new Date(),
    recNum: 0,
