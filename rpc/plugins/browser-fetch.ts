...
export const createMethodPlugin = (options: MethodFetchOptions = {}) => {
  const methodPlugin: PrimClientMethodPlugin = async (
    endpoint,
    jsonBody,
    jsonHandler,
    blobs = {}
    ...
    if (typeof options.headers === 'function') {
      headersOverride = options.headers()
    } else {
      headersOverride = options.headers || {}
    }
    ...
    if (blobList.length > 0 || jsonHandler.binary) {
      ...
      data.append(
        ...
        jsonHandler.binary
          ? new Blob([bodyStringish], { type: jsonHandler.mediaType })
          : bodyStringish
      )
      ...
      fetchOptions = {
        method: 'POST',
        headers: {
          'content-type': jsonHandler.mediaType ?? 'application/json',
          ...headersOverride,
        },
        // eslint-disable-next-line @typescript-eslint/no-unsafe-assignment
    ...
    if (resultContentType?.startsWith('multipart/form-data')) {
      ...
      const result = jsonHandler.parse(formData.get('rpc')) as
        | RpcAnswer
        | RpcAnswer[]
      formData.delete('rpc')
    ...
    const isBinaryLike = !!(
      resultContentType === jsonHandler.mediaType && jsonHandler.binary
    )
    const result = jsonHandler.parse(
      await (isBinaryLike ? fetchResult.arrayBuffer() : fetchResult.text())
    ) as RpcAnswer | RpcAnswer[]
    return { result }
  }
  return methodPlugin
}
