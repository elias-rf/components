...
import { EsterilizacaoInterna } from "./esterilizacao-interna.rpc";

describe("esterilizacaoInterna", () => {
  const knexDb = Knex({ client: MockClient });
  const rpc = EsterilizacaoInterna({
    oftalmo: knexDb,
  } as Connections);
  let tracker: Tracker;
  ...
  test("diario", async () => {
    ...
    const rsp = await rpc.esterilizacaoInternaDiario(
  ...
  test("mensal", async () => {
    ...
    const rsp = await rpc.esterilizacaoInternaMensal({ mes: "2020-01" });
  ...
  test("modelo", async () => {
    ...
    const rsp = await rpc.esterilizacaoInternaModelo(
