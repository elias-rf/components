...
describe("cliente", () => {
  ...
  const cliente = clienteRpc({ plano: knexDb } as Connections);
  ...
  test("possui schema", async () => {
    const rsp = await cliente.schema();
  ...
  test("read", async () => {
    tracker.on.select("CadCli").response(["ok"]);
    const rsp = await cliente.read({ id: ["10"] }, {});
    expect(rsp).toEqual("ok");
  ...
  test("del", async () => {
    ...
    const rsp = await cliente.del({ id: ["10"] }, {});
    expect(rsp).toEqual("ok");
  ...
  test("create", async () => {
    ...
    const rsp = await cliente.create({ rec: { CdCliente: "10" } }, {});
    expect(rsp).toEqual(["ok"]);
