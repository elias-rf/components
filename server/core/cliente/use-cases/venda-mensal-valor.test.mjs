...
import { vendaMensalValor } from './venda-mensal-valor.mjs'

describe('vendaMensalValor', () => {
  const tracker = getTracker()
  oftalmoDb.setDriver(knexMockMsql)
  planoDb.setDriver(knexMockMsql)
  const useCase = vendaMensalValor({ nfSaidaController })
  ...
  test('vendaMensalValor', async () => {
    ...
    expect(rsp).toEqual([{ '2020-01': '200', categoria: 'cat1' }])
  ...
  test('vendaMensalValor erro em argumentos', async () => {
    await expect(() =>
      /* @ts-ignore */
      useCase({
        inicio: '2020-01-01',
