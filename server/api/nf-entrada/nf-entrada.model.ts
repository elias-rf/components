...
const table = "nf_entrada";

export function nfEntradaModel(connections: TConnections): TNfEntradaRpc {
  ...
  const model: TNfEntradaRpc = {
    ...
    mutation: {
      ...
      async transferenciaCreate({
        ...
        const ordem_producao_id = await ordemProducao.query.fromControle({
          controle: controles[0],
        });

        for (const ctrl of controles) {
          const valid = await ordemProducao.query.ehControleValido({
            controle: ctrl,
          });
          if (!valid) throw new Error(`Controle ${ctrl} inválido`);
          const op_id = await ordemProducao.query.fromControle({
            controle: ctrl,
          });
        ...
        const { produto_plano_id } = await ordemProducao.query.produtoPlano({
        ...
        const fabricacao = await ordemProducao.query
        ...
        const expiracao = await ordemProducao.query.dataValidade({
          id: { ordem_producao_id },
        });
        ...
        const nf = await model.query.read({
          id: {
            filial_id: 1,
        ...
        await estoque.mutation.increment({
          id: { filial_id: "1", produto_plano_id: produto_plano_id || "" },
          quantidade,
        });

        await produtoEstatistica.mutation.increment({
          id: {
            filial_id: "1",
            mes: day().format("MM"),
            ano: day().format("YY"),
