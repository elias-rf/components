...
export class NfEntradaModel extends CrudModel {
  models: TModels;
  constructor({
    connections,
    models,
    ...
    super(connections[nf_entrada.database], nf_entrada);
    this.models = models;
    models.nfEntrada = this;
  }

  async transferenciaCreate({ controles }: { controles: string[] }) {
    const ordem_producao_id = await this.models.ordemProducao.fromControle({
      controle: controles[0],
    });

    for (const ctrl of controles) {
      const valid = await this.models.ordemProducao.ehControleValido({
        controle: ctrl,
      });
      if (!valid) throw new Error(`Controle ${ctrl} inválido`);
      const op_id = await this.models.ordemProducao.fromControle({
        controle: ctrl,
      });
    ...
    const { produto_plano_id }: any =
      await this.models.ordemProducao.produtoPlano({
    ...
    const fabricacao = await this.models.ordemProducao
    ...
    const expiracao = await this.models.ordemProducao.dataValidade({
      id: { ordem_producao_id },
    });
    ...
    const nf = await this.models.nfEntrada.read({
      id: {
        filial_id: 1,
    ...
    await this.models.nfEntrada.create({
      data: {
        filial_id: 1,
    ...
    await this.models.nfEntradaItem.create({
      data: {
        filial_id: 1,
        nota_id: parseInt(ordem_producao_id),
        serie_id: "XX",
    ...
    await this.models.estoque.increment({
      filter: {
        filial_id: "1",
        produto_plano_id: produto_plano_id.toString() || "",
      },
      increment: { estoque: quantidade },
    });

    await this.models.produtoEstatistica.increment({
      filter: {
        filial_id: "1",
        mes: day().format("MM"),
        ano: day().format("YY"),
    ...
    for (const ctrl of controles) {
      await this.models.produtoControle.create({
        data: {
          filial_id: 1,
      ...
      await this.models.nfEntradaControle.create({
        data: {
          filial_id: 1,
          nota_id: parseInt(ordem_producao_id),
          serie_id: "XX",
    ...
    await this.models.nfEntradaLog.create({
      data: {
        filial_id: 1,
        nota_id: parseInt(ordem_producao_id),
        serie_id: "XX",
