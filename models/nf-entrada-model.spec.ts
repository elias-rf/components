...
describe("NfEntradaModel", () => {
  ...
  it("list", async () => {
    ...
    const rsp = await nfEntradaModel.list({
      filter: { NroNf: "1" },
      sort: { NroNf: "asc" },
      select: ["NroNf"],
    });
  ...
  it("read", async () => {
    ...
    const rsp = await nfEntradaModel.read({
      id: { CdEmpresa: 1, Modelo: 1, NroNf: "1", Serie: "a" },
      select: ["NroNf"],
    });
    ...
    expect(knexMockHistory(tracker)).toEqual([
      {
        bindings: [1, 1, "1", "a"],
        sql: "select [NroNf] from [NfMestre] where ([NfMestre].[CdEmpresa] = @p0 and [NfMestre].[Modelo] = @p1 and [NfMestre].[NroNf] = @p2 and [NfMestre].[Serie] = @p3)",
      },
    ]);
  });
  ...
  it("transferenciaCreate", async () => {
    ...
    const rsp = await nfEntradaModel.transferenciaCreate({
    ...
    expect(knexMockHistory(tracker, ["sql"])).toEqual([
      ...
      {
        sql: "insert into [LotesNotaEntrada] ([CdFilial], [CdFornecedor], [CdProduto], [Modelo], [NumLote], [NumNota], [Quantidade], [Serie]) output inserted.[CdFilial], inserted.[NumNota], inserted.[Serie], inserted.[Modelo] values (@p0, @p1, @p2, @p3, @p4, @p5, @p6, @p7)",
      },
      {
        sql: "insert into [NfLogConferencia] ([CdFilial], [CdFornecedor], [Data], [Modelo], [NumNota], [Operacao], [Serie], [Usuario]) output inserted.[CdFilial], inserted.[NumNota], inserted.[Serie] values (@p0, @p1, @p2, @p3, @p4, @p5, @p6, @p7)",
      },
    ]);
    vi.useRealTimers();
  });
