...
import { nfSaidaFvModel } from "@/models";
import { CrudObjection } from "@/models/utils/crud-objection";
import { zsr } from "@/utils/zod/z-refine";
import { zd, zod } from "@/utils/zod/zod";
import { z } from "zod";

class NfSaidaModel extends CrudObjection {
  constructor() {
    super(NfSaida);
  }
  ...
  async vendaAnalitico({ inicio, fim }: { inicio: string; fim: string }) {
    ...
    const qryFullvision = nfSaidaFvModel.vendaAnalitico({
      inicio,
      fim,
    });
...
export const nfSaidaModel = new NfSaidaModel();
