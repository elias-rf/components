...
function Folder({ item, handleSelect }: { item: any; handleSelect: any }) {
  return (
    <div className="w-16 h-8">
      <div data-name={item.key} onClick={() => handleSelect(item.key)}>
        <Icon icon={folderIcon} className={"inline w-8 h-8"} /> {item.label}
      </div>
    </div>
  );
}
...
export function ListView({ data, selected = "/", dispatch }: ListViewProps) {
  ...
  return (
    <div className="flex space-x-4">
      <UpView selected={selected} dispatch={dispatch} />
      {selectedData.map((item: any) => {
        return (
          <Folder key={item.key} item={item} handleSelect={handleSelect} />
        );
      })}
    </div>
  );
