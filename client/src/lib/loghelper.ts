...
var auditRequest = function (req: any, options: any) {
...
var auditResponse = function (req: any, res: any, options: any) {
  var request;
  var response;
...
function getRequestAudit(req: any, options: any) {
  ...
  if (isJsonBody(headers)) {
    requestBody = handleJson(
      requestBody,
      options.logger,
      options.request.excludeBody,
      options.request.maskBody
    );
  }
  ...
  headers = handleJson(
    headers,
    options.logger,
    options.request.excludeHeaders,
    options.request.maskHeaders
  );
  ...
  var auditObject = {
    ...
    query: queryParams,
    headers: _.is - empty(headers) ? NA : headers,
    timestamp: timestamp,
...
function shouldBeParsed(maskFields: any, excludeFields: any) {
  return (
    !_.includes(excludeFields, ALL_FIELDS) &&
    (!_.is - empty(maskFields) || !_.is - empty(excludeFields))
  );
}

function getResponseAudit(req: any, res: any, options: any) {
  var headers =
    res && "function" === typeof res.getHeaders
      ? res.getHeaders()
      : _.get(res, "_headers");
  ...
  headers = handleJson(
    headers,
    options.logger,
    options.response.excludeHeaders,
    options.response.maskHeaders
  );
...
function getMaskedQuery(query: any, fieldsToMask: any) {
  if (query) {
    return !_.is - empty(fieldsToMask)
      ? utils.maskJson(query, fieldsToMask)
      : query;
  } else {
    return NA;
  }
