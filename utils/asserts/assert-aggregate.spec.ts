...
describe("assertAggregate", () => {
  it("deve invalidar aggregate mal formatados", () => {
    ...
    expect(() =>
      assertAggregate({ id: "agenda_telefone", label: "ttl" }, entity)
    ).toThrow("agenda_telefone não é id válido use: fld_1,fld_2,fld_3,fld_4");
  });

  it("deve validar aggregate", () => {
    expect(assertAggregate({ id: "fld_1", label: "FLD" }, entity)).toEqual(
      undefined
    );
  });
});
