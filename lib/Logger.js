    ...
    constructor(logDir = 'logs', env = environment) {
        ...
        this.errorLogger = createLogger({
            ...
            transports: [
                new transports.Console({
                    levels: 'error',
                    format: format.combine(format.colorize(), format.printf(error => `${error.timestamp} ${error.level}: ${error.message}`)),
                }),
                new DailyRotate({
        ...
        this.warnLogger = createLogger({
            ...
            transports: [
                new transports.Console({
                    levels: 'warn',
                    format: format.combine(format.colorize(), format.printf(warn => `${warn.timestamp} ${warn.level}: ${warn.message}`)),
                }),
                new DailyRotate({
